...
#[command(
    name = "binance",
    about = "Binance trading CLI — spot and perpetual futures"
)]
struct Cli {
    #[command(subcommand)]
    ...
    /// Example: binance --json '{"command":"buy","symbol":"ETH","amount":0.1,"price":2000}'
    #[arg(long)]
    json: Option<String>,
}
...
enum Commands {
    ...
    /// List open orders (spot and futures)
...
async fn main() -> Result<()> {
    ...
    let command = match cli.command {
        Some(cmd) => cmd,
        None => {
            <Cli as clap::Parser>::parse_from(["binance", "--help"]);
            unreachable!()
        }
    };
