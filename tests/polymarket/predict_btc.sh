...
# on Polymarket prediction markets. Every polymarket call uses --json mode.
#
# Workflow:
...
QUOTE2=$(ft "{\"command\":\"quote\",\"market\":\"$MARKET_SLUG\"}")
...
RESULT=$(ft "{\"command\":\"sell\",\"market\":\"$MARKET_SLUG\",\"outcome\":\"Yes\",\"amount\":$BUY_AMOUNT,\"price\":$SELL_PRICE}")
...
BALANCE_RESULT=$(ft '{"command":"balance"}')
...
if [[ "$USDC_BALANCE" == "0" || "$USDC_BALANCE" == "0.0" ]]; then
    ...
    RESULT=$(ft "{\"command\":\"withdraw\",\"asset\":\"USDC\",\"amount\":$USDC_BALANCE,\"to\":\"base\"}")
